...
using rm = CorelyFramework.Resources.UI.Distributions;
...
namespace CorelyFramework.UI
{
    public partial class Distributions : System.Windows.Window
    {
        ...
        public Distributions(Func<List<DistributionSettings>> loadSettings, Action<List<DistributionSettings>> saveSettings)
        {
            ...
            {
                CorelyFramework.Core.Message.Show(rm.failLoad + Environment.NewLine + ex.ToString());
            }
        }
        ...
        private void Dist_OnSave()
        {
            try
            {
                ...
                CorelyFramework.Core.Message.Show(rm.distSettingsSaved);
            }
            catch (Exception ex)
            {
                CorelyFramework.Core.Message.Show(rm.failSave + Environment.NewLine + ex.ToString());
            }
        }
